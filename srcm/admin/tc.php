...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add') {
    try {
        ...
        $stmt->execute([
            clean($_POST['tc_number']),
            clean($_POST['student_name']),
            clean($_POST['father_name']),
            clean($_POST['class']),
            clean($_POST['stream']),
            clean($_POST['admission_year']),
            clean($_POST['leaving_year']),
            $_POST['issue_date'],
        ]);
        $msg = 'success|TC Record added!';
    } catch (Exception $e) {
        $msg = 'error|TC Number already exists!';
    }
}
...
$editItem = null;
if (isset($_GET['edit'])) {
    $s = $db->prepare("SELECT * FROM tc_records WHERE id=?");
    $s->execute([$_GET['edit']]);
    $editItem = $s->fetch();
}
...
<div class="admin-main">
  ...
  <?php if ($msgText): ?><div class="form-<?= $msgType ?> alert-auto" style="margin-bottom:20px;padding:12px 18px;border-radius:8px;"><?= $msgType === 'success' ? 'Success:' : 'Error:' ?> <?= $msgText ?></div><?php endif; ?>
    ...
    <table class="admin-table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><?= $t['issue_date'] ? formatDate($t['issue_date']) : 'N/A' ?></td>
